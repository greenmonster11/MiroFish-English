...
const renderGraph = () => {
  ...
  // Count edges between each pair of nodes and collect self-loop edges
  tempEdges.forEach(e => {
  ...
  tempEdges.forEach(e => {
    ...
    // Compute curvature: spread apart when multiple edges exist; single edge is a straight line
    let curvature = 0
    if (totalCount > 1) {
      ...
      // If the edge direction is reversed relative to the canonical direction, flip the curvature
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravitational pull toward the center to cluster isolated node groups in the central area
  ...
  const getLinkPath = (d) => {
    ...
    // Offset perpendicular to the link direction, proportional to distance, to keep curves visibly distinct
    // More edges means a larger proportion of offset relative to distance
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect whether an actual drag has started (movement exceeds threshold)
      ...
      .on('end', (event, d) => {
        // Only let the simulation wind down if an actual drag occurred
        if (d._isDragging) {
...
// Watch for edge label visibility toggle

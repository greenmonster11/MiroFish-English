...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interview counts (e.g., "5 / 9 simulated agents")
    ...
    // Parse individual selection reasons for each agent
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          // Save the previous agent's reason
          ...
          // Continuation of reason text (excluding closing summary paragraphs)
          currentReason.push(line.trim())
        }
      }
      
      // Save the last agent's reason
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    // Extract each agent's interview record
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (backward-compatible with old format: only one platform tag present)
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer exists; copy to Twitter display only when it is not placeholder text
          ...
          // No platform tags at all (very old format), treat the entire text as the answer
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          ...
          }, expandedEntities.value ? `Collapse ▲` : `Expand all ${props.result.entities.length} agents ▼`)
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check whether the text is platform placeholder text
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: first question returns the full answer, others return empty
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          !showTabs.value && h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.facts.length} items total`)
          ]),
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.edges.length} items total`)
          ]),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.nodes.length} nodes total`)
          ]),
...
const renderMarkdown = (content) => {
  ...
  // Clean up <br> between block-level closing tags and following paragraph opening tags
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: keep numbers incrementing when single-item <ol> tags are separated by paragraph content
...
const extractFinalContent = (response) => {
  ...
  // Legacy fallback removed - backend now uses English "Final Answer:"
  ...
  // If content is long and contains markdown formatting, try to remove the thinking process and return

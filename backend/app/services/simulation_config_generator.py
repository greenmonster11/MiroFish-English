...
class SimulationConfigGenerator:
    ...
    entity information to automatically generate optimal simulation parameters.
    ...
    def generate_config(self, simulation_id: str, project_id: str, graph_id: str, simulation_requirement: str, document_text: str, entities: List[EntityNode], enable_twitter: bool = True, enable_reddit: bool = True, progress_callback: Optional[Callable[[int, int, str], None]] = None) -> SimulationParameters:
        """
        ...
        """
        logger.info(f"Starting smart config generation: simulation_id={simulation_id}, entity_count={len(entities)}")
        ...
        context = self._build_context(simulation_requirement=simulation_requirement, document_text=document_text, entities=entities)
        ...
        for batch_idx in range(num_batches):
            ...
            batch_configs = self._generate_agent_configs_batch(context=context, entities=batch_entities, start_idx=start_idx, simulation_requirement=simulation_requirement)
        ...
        if enable_twitter:
            twitter_config = PlatformConfig(platform="twitter", recency_weight=0.4, popularity_weight=0.3, relevance_weight=0.3, viral_threshold=10, echo_chamber_strength=0.5)

        if enable_reddit:
            reddit_config = PlatformConfig(platform="reddit", recency_weight=0.3, popularity_weight=0.4, relevance_weight=0.3, viral_threshold=15, echo_chamber_strength=0.6)

        # Build final parameters
        params = SimulationParameters(simulation_id=simulation_id, project_id=project_id, graph_id=graph_id, simulation_requirement=simulation_requirement, time_config=time_config, agent_configs=all_agent_configs, event_config=event_config, twitter_config=twitter_config, reddit_config=reddit_config, llm_model=self.model_name, llm_base_url=self.base_url, generation_reasoning=" | ".join(reasoning_parts))
...
- Morning 6-8am: gradually becoming active (activity coefficient 0.4)
- Work hours 9am-6pm: moderately active (activity coefficient 0.7)
- Evening 7-10pm: peak period (activity coefficient 1.5)
- After 11pm: activity declining (activity coefficient 0.5)
- General pattern: low activity at dawn, gradually increasing in morning, moderate during work hours, peak in evening
- **Important**: The example values below are for reference only. You need to adjust the specific time periods based on the nature of the event and characteristics of participating groups
  - For example: student group peak may be 9-11pm; media active all day; official institutions only during work hours
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format. Time configuration must follow Chinese daily schedule habits. Always respond in English."
...
{{
    ...
    "reasoning": "<brief explanation>"
        ...
        system_prompt = "You are a public opinion analysis expert. Return pure JSON format. Note that poster_type must exactly match the available entity types. Always respond in English."
...
- **Schedule follows Chinese daily patterns**: almost no activity midnight-5am, most active 7-10pm
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON. Configuration must follow Chinese daily schedule habits. Always respond in English."

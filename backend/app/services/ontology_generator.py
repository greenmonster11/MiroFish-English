...
- We need to simulate the reactions of various parties in public opinion events and the paths of information propagation
...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual. When a person does not belong to any more specific person type, classify them here.
...
B. **Specific types (8, designed based on text content)**:
   ...
   - Example: For academic events, you can have `Student`, `Professor`, `University`
   - Example: For business events, you can have `Company`, `CEO`, `Employee`

**Why fallback types are needed**:
- Various people appear in texts, such as "elementary school teachers", "passersby", "certain netizens"
...
- Person: Any individual (used when no specific type above applies)
...
5. Attribute names cannot use name, uuid, group_id, or other reserved words; use full_name, org_name, etc. instead
"""
